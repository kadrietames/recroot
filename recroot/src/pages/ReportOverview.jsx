  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.error('Operational error pulling analytics:', err)
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 pt-1 pb-6 text-left">
        ...
        <div className="w-full mb-5 flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
          ...
          <div className="flex items-center gap-2 sm:mt-6">
            <button className="border border-slate-300 text-slate-600 text-xs font-semibold px-4 py-1.5 rounded-lg hover:bg-slate-50 transition cursor-pointer">
              Share
            </button>
            <button className="bg-[#163C6B] text-white text-xs font-semibold px-4 py-1.5 rounded-lg hover:opacity-90 transition shadow-xs cursor-pointer">
              Download
            </button>
          </div>
        </div>
        ...
        <div className="flex items-center justify-start gap-1.5 sm:gap-2 mb-6 bg-transparent py-1 overflow-x-auto select-none">
          <div className="flex items-center gap-1.5 shrink-0">
            <span className="w-7 h-7 flex items-center justify-center rounded-full bg-emerald-500 text-white font-semibold text-xs">✓</span>
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            <span className="w-7 h-7 flex items-center justify-center rounded-full bg-emerald-500 text-white font-medium text-xs">✓</span>
        ...
        {loading ? (
          <div className="py-12 text-center sm:text-left">
            <p className="text-slate-400 text-xs font-medium animate-pulse">Assembling parsed data metrics...</p>
          </div>
        ) : error ? (
          <div className="p-4 bg-rose-50 border border-rose-100 rounded-xl text-rose-600 text-xs font-semibold max-w-2xl">
            ⚠️ {error}
          </div>
        ) : (
          <div className="space-y-5">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 max-w-2xl">
              <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-4 shadow-2xs">
                <div className="flex items-center justify-between mb-1">
                  <p className="text-slate-500 text-[10px] font-bold uppercase tracking-wider">Matched Skills</p>
              ...
              <div className="bg-rose-50 border border-rose-100 rounded-xl p-4 shadow-2xs">
                <div className="flex items-center justify-between mb-1">
                  <p className="text-slate-500 text-[10px] font-bold uppercase tracking-wider">Missing Skills</p>
              ...
              <div className="bg-slate-50 border border-slate-200/60 rounded-xl p-4 shadow-2xs">
                <div className="flex items-center justify-between mb-1">
                  <p className="text-slate-500 text-[10px] font-bold uppercase tracking-wider">Experience</p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-2xl">
              ...
              <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-2xs flex flex-col items-center justify-center text-center">
                  ...
                  <div className="flex flex-col gap-2.5">
                    {matchBreakdown.map((item, index) => (
                      <div key={index} className="flex items-center gap-3">
                        <p className="text-slate-500 text-[10px] w-24 shrink-0 font-medium">{item.label}</p>
                        ...
                        <p className="text-[#0f2537] text-[10px] font-mono font-bold w-8 text-right">{item.value}%</p>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                {interviews.length > 0 && (
                  <div className="mt-4 pt-3 border-t border-slate-100">
                    <p className="text-slate-400 text-[10px] font-medium">
                      ✨ {interviews.length} interview session{interviews.length > 1 ? 's' : ''} saved in history
                    </p>
                  </div>
                )}
              </div>
