...
function InterviewPrep() {
  ...
  // ===== LIVE BACKEND DATA OBJECT STATES =====
  ...
  // ===== EFFECT HOOK: INITIAL API DATA MOUNT =====
  useEffect(() => {
    async function fetchDashboardMetadata() {
      try {
        // 1. Fetch resumes to verify the user has a parsed document ready
        const resumeData = await getMyResumes();
        if (resumeData && resumeData.length > 0) {
          const currentResume = resumeData[0]; // Take primary uploaded file
        ...
        // 2. Fetch past session results logs
  ...
  // ===== INTERACTION INTERFACES: POST REQ CONNECTS =====
  ...
  return (
    <DashboardLayout activePage="interview-prep">
      <div className="w-full max-w-5xl mx-auto p-4 md:p-6 lg:p-8 font-sans space-y-6 relative">
        ...
        {activeStep === 1 && (
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
            ...
            {/* Left Workspace Layout Column */}
            <div className="lg:col-span-7 space-y-6">
              
              {/* Practice Type Card Selector Matrix */}
              <div className="bg-white rounded-xl border border-slate-200/70 p-5 shadow-2xs">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  ...
                  {/* Card 1: AI Interview */}
                  <div 
                    onClick={() => setSelectedPractice('AI Interview')}
                    className={`cursor-pointer rounded-xl p-4 border text-center relative flex flex-col items-center justify-between min-h-[140px] transition-all duration-300 select-none group
                      ${selectedPractice === 'AI Interview' 
                        ? 'border-blue-600 bg-blue-50/10 -translate-y-0.5 shadow-xs' 
                        : 'border-slate-200 bg-white hover:-translate-y-0.5 hover:border-slate-300'
                      }`}
                  >
                    <div className="text-xl text-blue-600 mt-1">🤖</div>
                    <div className="my-2">
                      <h4 className="text-xs font-bold text-slate-800">AI Interview</h4>
                      <p className="text-[10px] text-slate-400 leading-tight max-w-[110px] mx-auto mt-1">Simulate a real interview with AI</p>
                    </div>
                    <div className="mb-1">
                      {selectedPractice === 'AI Interview' ? (
                  ...
                  {/* Card 2: Technical */}
                  <div 
                    onClick={() => setSelectedPractice('Technical Assessment')}
                    className={`cursor-pointer rounded-xl p-4 border text-center relative flex flex-col items-center justify-between min-h-[140px] transition-all duration-300 select-none group
                      ${selectedPractice === 'Technical Assessment' 
                        ? 'border-blue-600 bg-blue-50/10 -translate-y-0.5 shadow-xs' 
                        : 'border-slate-200 bg-white hover:-translate-y-0.5 hover:border-slate-300'
                      }`}
                  >
                    <div className="text-xl text-rose-500 mt-1">📅</div>
                    <div className="my-2">
                      <h4 className="text-xs font-bold text-slate-800">Technical</h4>
                      <p className="text-[10px] text-slate-400 leading-tight max-w-[110px] mx-auto mt-1">Design, coding & technical Q&A</p>
                    </div>
                    <div className="mb-1">
                      {selectedPractice === 'Technical Assessment' ? (
                        <div className="w-4 h-4 rounded-full bg-blue-600 flex items-center justify-center text-[9px] text-white font-bold">✓</div>
                  ...
                  {/* Card 3: Behavioural */}
                  <div 
                    onClick={() => setSelectedPractice('Behavioral Matrix')}
                    className={`cursor-pointer rounded-xl p-4 border text-center relative flex flex-col items-center justify-between min-h-[140px] transition-all duration-300 select-none group
                      ${selectedPractice === 'Behavioral Matrix' 
                        ? 'border-blue-600 bg-blue-50/10 -translate-y-0.5 shadow-xs' 
                        : 'border-slate-200 bg-white hover:-translate-y-0.5 hover:border-slate-300'
                      }`}
                  >
                    <div className="text-xl text-amber-500 mt-1">📋</div>
                    <div className="my-2">
                      <h4 className="text-xs font-bold text-slate-800">Behavioural</h4>
                      <p className="text-[10px] text-slate-400 leading-tight max-w-[110px] mx-auto mt-1">Practice soft skills questions</p>
                    </div>
                    <div className="mb-1">
                      {selectedPractice === 'Behavioral Matrix' ? (
                        <div className="w-4 h-4 rounded-full bg-blue-600 flex items-center justify-center text-[9px] text-white font-bold">✓</div>
              ...
              {/* Dynamic About Role Configuration Form */}
              <AboutRole 
        ...
        {/* --- STEP 2: QUESTIONS GENERATION PANEL --- */}
        {activeStep === 2 && (
        ...
        {/* --- STEP 3: ANALYTICS PERFORMANCE SCOREBOARD --- */}
        {activeStep === 3 && (
          <div className="max-w-4xl mx-auto space-y-6">
            
            {/* Top Score Matrix Metrics Layout Blocks */}
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div className="bg-[#e8f7f2] border border-[#d1f0e4] rounded-xl p-4 text-center">
                ...
                <h3 className="text-xl font-black text-[#047857] mt-1">17</h3>
              </div>
              <div className="bg-[#fdf2f2] border border-[#fde2e2] rounded-xl p-4 text-center">
                ...
                <h3 className="text-xl font-black text-[#b91c1c] mt-1">3</h3>
              </div>
              <div className="bg-[#e8f7f2] border border-[#d1f0e4] rounded-xl p-4 text-center">
                <p className="text-[9px] uppercase font-bold tracking-wider text-[#10b981]">Overall Score</p>
                <h3 className="text-xl font-black text-[#047857] mt-1">
                  {historicalLogs.length > 0 ? `${Math.round(historicalLogs.reduce((acc, current) => acc + current.score, 0) / historicalLogs.length)}%` : '85%'}
                </h3>
              </div>
            </div>

            {/* Performance Analytics Tracking Overview Block */}
            <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-2xs">
              ...
              <div className="h-40 border-b border-l border-slate-200 relative mt-6 flex items-end justify-between px-6 sm:px-14">
                ...
                {/* Trend Graph plots populated visually */}
                <div className="flex flex-col items-center gap-1 w-12 relative">
                  <span className="text-[9px] font-bold text-slate-500 absolute -top-4">25%</span>
                  <div className="w-2.5 h-10 bg-slate-100 rounded-t-xs" />
                  <span className="text-[9px] font-medium text-slate-400 mt-1">week 1</span>
                </div>
                <div className="flex flex-col items-center gap-1 w-12 relative">
                  <span className="text-[9px] font-bold text-slate-500 absolute -top-4">40%</span>
                  <div className="w-2.5 h-14 bg-slate-100 rounded-t-xs" />
                  <span className="text-[9px] font-medium text-slate-400 mt-1">week 2</span>
                </div>
                <div className="flex flex-col items-center gap-1 w-12 relative">
                  <span className="text-[9px] font-bold text-slate-500 absolute -top-4">54%</span>
                  <div className="w-2.5 h-20 bg-slate-100 rounded-t-xs" />
                  <span className="text-[9px] font-medium text-slate-400 mt-1">Week 3</span>
                </div>
                <div className="flex flex-col items-center gap-1 w-12 relative">
                  <span className="text-[9px] font-bold text-emerald-600 absolute -top-4">75%</span>
                  <div className="w-2.5 h-28 bg-emerald-400 rounded-t-xs" />
                  <span className="text-[9px] font-bold text-slate-700 mt-1">This Week</span>
                </div>
              </div>
            </div>
        ...
        {/* --- BOTTOM ROW: RECOMMENDED FOR YOU TRACKER --- */}
      ...
      {/* --- MODAL SIDE SLIDE HISTORY DRAWER --- */}
      {showHistoryDrawer && (
